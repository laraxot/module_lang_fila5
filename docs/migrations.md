...
## Merge Git e errore parse

Se compaiono **due** istruzioni `return new class extends ...` consecutive (tipico di merge non ripulito), PHP genera `ParseError` e PHPStan non completa l'analisi di `Modules`. Va lasciata una sola dichiarazione di classe anonima. Stato analisi condiviso tra agenti: [phpstan-analysis](../../../../docs/chat/phpstan-analysis.txt).
